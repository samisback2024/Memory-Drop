...
export const MomentSeenList: React.FC<MomentSeenListProps> = ({ isOpen, onClose, momentId, viewCount }) => {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-900 w-full sm:max-w-md rounded-t-2xl max-h-[70vh] overflow-y-auto animate-slide-up">
        <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800 sticky top-0 bg-white dark:bg-gray-900">
          <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-1.5">
            <Eye size={16} className="text-gray-400 dark:text-gray-500" aria-hidden="true" />
            Seen by {viewCount}
          </h2>
          <button type="button" onClick={onClose} aria-label="Close" className="p-1 rounded-lg text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
        ...
        <div className="py-1">
          {loading ? (
            ...
            <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-6">No one has seen this moment yet.</p>
          ) : (
            viewers.map(v => (
              <div key={v.viewer_id} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{v.display_name || v.username}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 truncate">@{v.username}</p>
                </div>
                <span className="text-xs text-gray-400 dark:text-gray-500 flex-shrink-0">{formatRelativeTime(v.viewed_at)}</span>
              </div>
            ))
          )}
        </div>
