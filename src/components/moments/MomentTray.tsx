...
export const MomentTray: React.FC<MomentTrayProps> = ({ onCreate, onOpenAuthor, refreshKey }) => {
  ...
  if (loading) {
    return (
      <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
        {[0, 1, 2, 3].map(i => (
          <div key={i} className="w-16 h-16 rounded-full bg-gray-100 animate-pulse flex-shrink-0" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1" role="list" aria-label="Moments">
      ...
      >
        <span className="w-16 h-16 rounded-full border-2 border-dashed border-purple-300 bg-purple-50/60 flex items-center justify-center">
          <Plus size={22} className="text-purple-500" aria-hidden="true" />
        </span>
        <span className="text-xs text-gray-600 truncate w-full text-center">Add Moment</span>
      </button>
