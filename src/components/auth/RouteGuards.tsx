...
const isAuthenticated = (user: unknown, isDemo: boolean) => Boolean(user) || isDemo;
...
export const AuthProtectedRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { user, isDemo, loading, needsProfileCompletion } = useAuth();
  const location = useLocation();

  if (loading) return <AuthSpinner />;
  if (!isAuthenticated(user, isDemo)) return <Navigate to="/login" state={{ from: location }} replace />;
...
export const RootRedirect: React.FC = () => {
  ...
  if (loading) return <AuthSpinner />;
  if (!isAuthenticated(user, isDemo)) return <Navigate to="/login" replace />;
