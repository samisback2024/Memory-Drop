-- Memory Drop — Phase 17: a real "Moments" count for the profile stats
-- row, replacing the always-zero "Stories" placeholder StatsRow.tsx has
-- shown since Phase 2. Also widens the opt-in public-stats list
-- (user_settings.visible_stats) to include it, same privacy treatment
-- every other extra profile stat already gets — hidden from strangers
-- unless the profile owner explicitly turns it on in Settings → Privacy.
-- Run once, after supabase/phase16_feed_retention_window.sql, in the
-- Supabase SQL editor.
