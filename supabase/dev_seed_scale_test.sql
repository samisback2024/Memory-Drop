-- Memory Drop — Scale-test seed data (Phase 10f, second half).
--
-- ⚠ NOT a schema migration — do not add this to your normal migration
-- run. This creates ~60 fake accounts and thousands of fake content
-- rows for load/consistency testing (1000 Drops, 500 Moments, 500
-- Capsules, ~900 follows, ~4500 comments, ~6000 likes/reactions, ~600
-- saves — matching the brief's "1000 drops, 500 moments, 500 capsules,
-- many followers, many comments, many reactions, many bookmarks").
-- Run it only against a local/staging project you're happy to fill
-- with junk and wipe afterward — never against production.
--
...
-- moments/follows/comments/likes/saves/... all the way down) via the
-- same `on delete cascade` chain real account deletion already relies
-- on (see delete_my_account() in phase8_settings.sql) — nothing extra
-- to clean up by hand.
--
...
-- ---------------------------------------------------------------------------
-- 1. Fake accounts (60 users) — inserting into auth.users and letting
--    the existing handle_new_user() trigger (phase1_auth.sql) create
--    the matching profiles row from raw_user_meta_data, exactly like a
--    real sign-up does. profiles.id has a hard FK to auth.users.id, so
--    there's no way to create standalone fake profiles without this.
-- ---------------------------------------------------------------------------
...
-- Pool of fake user ids, indexed 1..60 for deterministic-but-varied
...
-- ---------------------------------------------------------------------------
-- 2. Follows (~900 rows) — each fake user follows ~15 random others.
--    status is left for the existing follow-request trigger to compute
--    (phase3_social_graph.sql) — every seeded profile is public by
--    default, so it resolves to 'accepted' automatically, same as a
--    real follow of a public account would.
-- ---------------------------------------------------------------------------
...
select
  (select id from _seed_users where rn = ((i % 60) + 1)),
  'text',
...
-- ---------------------------------------------------------------------------
-- 4. Moments (500) — text-only, duration_hours must be exactly 12/24/48
...
-- ---------------------------------------------------------------------------
-- 5. Capsules (500) — memory_types must be non-empty; unlock_date must
...
select
  (select id from _seed_users where rn = ((gen.i % 60) + 1)),
  ...
  gen.created_at
from (select i, now() - (random() * 400 || ' days')::interval as created_at from generate_series(1, 500) as i) as gen;
...
select p.id, (select id from _seed_users where rn = (1 + floor(random() * 60)::int)), 'Scale-test comment on drop.'
...
select c.id, (select id from _seed_users where rn = (1 + floor(random() * 60)::int)), 'Scale-test comment on capsule.'
...
select cm.id, (select id from _seed_users where rn = (1 + floor(random() * 60)::int)), (array['❤️', '😂', '👍', '🔥', '🙏'])[1 + floor(random() * 5)::int]
...
select cc.id, (select id from _seed_users where rn = (1 + floor(random() * 60)::int)), (array['❤️', '😂', '👍', '🔥', '🙏'])[1 + floor(random() * 5)::int]
...
select p.id, (select id from _seed_users where rn = (1 + floor(random() * 60)::int))
...
select c.id, (select id from _seed_users where rn = (1 + floor(random() * 60)::int))
